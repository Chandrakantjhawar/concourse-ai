...
## 🎯 Chosen Vertical
This project tackles the **Venue & Operations Management** vertical (with crossover into **Fan Experience**). It is a unified platform connecting fans, volunteers, transit authorities, and command center operators into a single AI-driven ecosystem.
...
2. **Operations Command Center**: IoT sensors (simulated via API) send telemetry. The backend aggregates this and prompts Gemini to act as a security analyst, returning a formatted briefing with priority-ranked actions.
3. **Volunteer Digest**: Staff receive shift briefings generated by combining current weather, transit status, and relevant standard operating procedures (SOPs).
4. **Transit Advisor**: Fans input their origin, and Gemini evaluates current parking lot capacity, shuttle ETAs, and sustainability goals to recommend the optimal route.

## 📝 Assumptions Made
- **IoT Telemetry**: We assume stadium turnstiles and cameras are feeding occupancy data to the `/api/crowd/telemetry` endpoint.
